/*jQuery Form to Form Wizard (Oct 1st, 2010)
...
function formtowizard(options){
	this.setting=jQuery.extend({persistsection:true, revealfx:['slide', 500], oninit:function(){}, onpagechangestart:function(){}}, options)
...
formtowizard.prototype={
	...
	loadsection:function(rawi, isinit){
		var thiswizard=this
  	//doload Boolean checks to see whether to load next section (true if isinit param is true or onpagechangestart() event handler doesn't return false)
		var doload=isinit || this.setting.onpagechangestart(jQuery, this.currentsection, this.sections.$sections.eq(this.currentsection))
		...
		if (this.currentsection!=i && i<this.sections.count && doload){ //if next section to show isn't the same as the current section shown
			...
			this.paginatediv.$status.text("Page "+(i+1)+" sur "+this.sections.count) //update current page status text
		...
		jQuery(function($){ //on document.ready
			...
			$sections.each(function(i){ //loop through $sections again
				if (setting.revealfx[0]=="slide"){
					$(this).data('page', i).css({position:'absolute', top:0, left:maxfieldsetwidth*i}).appendTo($sectionswrapper_inner) //set fieldset position to "absolute" and move it to inside sectionswrapper_inner DIV
				}
				//create each "step" DIV and add it to main Steps Container:
				var $thestep=$('<div class="step disabledstep" />').data('section', i).html('Etape '+(i+1)+'<div class="smalltext">'+$(this).find('legend:eq(0)').text()+'</div>').appendTo($stepsguide)
			...
			$theform.prepend($stepsguide) //add Steps Container to beginning of form
			var $thesteps=$stepsguide.find('div.step')
			//create pagination DIV and add it to end of form:
			var $paginatediv=$('<div class="formpaginate" style="overflow:hidden;"><span class="prev" style="float:left">Précédent</span> <span class="status">Etape 1 sur </span> <span class="next" style="float:right">Suivant</span></div>')
