     		...
     		if ($field['link']) {
     			...
     			if (is_Array($field['target'])) {
     				...
     				/*$Columns .= "{ 'mData': '".$field['field']."', sDefaultContent: '','mRender': function ( data, type, full ) {
        																		return '<a href=\"".$field['target']['controller']."/".$field['target']['action']."/id/'+data+'\">'+data+'</a>';	
      																			}
						},";*/
    	 			/*On enlève pour faire figurer les datas dans le lien
    	 			 * Permet dans certains cas ou le data est différent de l'ID d'avoir les 2 sous le coude mis en parametre dans l'url
    	 			 */
    	 			 /* $Columns .= "{ 'mData': '".$field['field']."', sDefaultContent: '','mRender': function ( data, type, full ) {
     				...
     				$Columns .= "{ 'mData': '".$field['field']."', sDefaultContent: '','mRender': function ( data, type, full ) {
        							return '<a href=\"".$ctrl_act."/id/'+row.id+'/data/'+data+'\" style=\"width:".$field['width'].";\">'+data+'</a>';
     							}
								},";
     			...
     			else
     			{
	     			...
	     			/*$Columns .= "{ 'mData': '".$field['field']."',
		     			...
		     			$Columns .= "{ 'mData': '".$field['field']."',
		     							...
		     							'mRender': function ( data, type, row )
		     									{
		        								return '<a href=\"".$this->_request.$target."/id/'+row.id+'/data/'+data+'\">'+data+'</a>';
		      									}
								},";
        ...
        $script = "$(document).ready(function() {
        		 ...
        		 $('#grille').dataTable( {
					...
					'oLanguage': 
					 ...
					 'tableTools': {'sSwfPath': '/public/layouts/frontoffice/swf/copy_csv_xls_pdf.swf',
    				 				'aButtons': [
						                 {
						                    'sExtends': 'collection',
						                    'sButtonText': 'Export',
