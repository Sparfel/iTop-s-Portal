...
/**
 * @see Centurion_ZFDebug_Controller_Plugin_Debug_Plugin_Text
 */
require_once 'Centurion/ZFDebug/Controller/Plugin/Debug/Plugin/Text.php';
 ...
 */
class Centurion_ZFDebug_Controller_Plugin_Debug extends Zend_Controller_Plugin_Abstract
{
    /**
    ...
    protected $_options = array(
        'plugins'           => array(
            ...
            'Time' => null,
            'Memory' => null),
        'z-index'           => 10255, //$MOD Emmanuel Lozachmeur, to be sure to be always viewed
    ...
    public function __construct($options = null)
    {
         ...
         */
        $version = new Centurion_ZFDebug_Controller_Plugin_Debug_Plugin_Text();
     ...
     *
     * @param array $options
     * @return Centurion_ZFDebug_Controller_Plugin_Debug
     */
     ...
     *
     * @param Centurion_ZFDebug_Controller_Plugin_Debug_Plugin_Interface
     * @return Centurion_ZFDebug_Controller_Plugin_Debug
     */
    public function registerPlugin(Centurion_ZFDebug_Controller_Plugin_Debug_Plugin_Interface $plugin)
    {
     ...
     * @return Centurion_ZFDebug_Controller_Plugin_Debug_Plugin_Interface
     */
    ...
    public function dispatchLoopShutdown()
    {
        ...
        foreach ($this->_plugins as $plugin)
        {
            ...
            /* @var $plugin Centurion_ZFDebug_Controller_Plugin_Debug_Plugin_Interface */
            $html .= '<div id="Centurion_ZFDebug_' . $plugin->getIdentifier()
                  . '" class="Centurion_ZFDebug_panel">' . $panel . '</div>';
        }

        $html .= '<div id="Centurion_ZFDebug_info">';
        ...
        foreach ($this->_plugins as $plugin)
        {
            ...
            /* @var $plugin Centurion_ZFDebug_Controller_Plugin_Debug_Plugin_Interface */
            $html .= '<span class="Centurion_ZFDebug_span clickable" onclick="ZFDebugPanel(\'Centurion_ZFDebug_' . $plugin->getIdentifier() . '\');">';
        ...
        $html .= '<span class="Centurion_ZFDebug_span Centurion_ZFDebug_last clickable" id="Centurion_ZFDebug_toggler" onclick="ZFDebugSlideBar()">&#171;</span>';
    ...
    protected function _loadPlugins()
    {
    	foreach($this->_options['plugins'] as $plugin => $options) {
    	    ...
    	    if (in_array($plugin, Centurion_ZFDebug_Controller_Plugin_Debug::$standardPlugins)) {
    	        // standard plugin
                $pluginClass = 'Centurion_ZFDebug_Controller_Plugin_Debug_Plugin_' . $plugin;
    	    } else {
    ...
    protected function _headerOutput() {
        ...
        return ('
            <style type="text/css" media="screen">
                #Centurion_ZFDebug_debug { font: 11px/1.4em Lucida Grande, Lucida Sans Unicode, sans-serif; position:fixed; bottom:5px; left:5px; color:#000; z-index: ' . $this->_options['z-index'] . ';}
                #Centurion_ZFDebug_debug ol {margin:10px 0px; padding:0 25px}
                #Centurion_ZFDebug_debug li {margin:0 0 10px 0;}
                #Centurion_ZFDebug_debug .clickable {cursor:pointer}
                #Centurion_ZFDebug_toggler { font-weight:bold; background:#BFBFBF; }
                .Centurion_ZFDebug_span { border: 1px solid #999; border-right:0px; background:#DFDFDF; padding: 5px 5px; }
                .Centurion_ZFDebug_last { border: 1px solid #999; }
                .Centurion_ZFDebug_panel { text-align:left; position:absolute;bottom:21px;width:600px; max-height:400px; overflow:auto; display:none; background:#E8E8E8; padding:5px; border: 1px solid #999; }
            ...
            <script type="text/javascript" charset="utf-8">
                ...
                function ZFDebugCollapsed() {
                    if ('.$collapsed.' == 1) {
                        ZFDebugPanel();
                        jQuery("#Centurion_ZFDebug_toggler").html("&#187;");
                        return jQuery("#Centurion_ZFDebug_debug").css("left", "-"+parseInt(jQuery("#Centurion_ZFDebug_debug").outerWidth()-jQuery("#Centurion_ZFDebug_toggler").outerWidth()+1)+"px");
                    }
                }
                
                function ZFDebugPanel(name) {
                    jQuery(".Centurion_ZFDebug_panel").each(function(i){
                ...
                function ZFDebugSlideBar() {
                    if (jQuery("#Centurion_ZFDebug_debug").position().left > 0) {
                        document.cookie = "ZFDebugCollapsed=1;expires=;path=/";
                        ZFDebugPanel();
                        jQuery("#Centurion_ZFDebug_toggler").html("&#187;");
                        return jQuery("#Centurion_ZFDebug_debug").animate({left:"-"+parseInt(jQuery("#Centurion_ZFDebug_debug").outerWidth()-jQuery("#Centurion_ZFDebug_toggler").outerWidth()+1)+"px"}, "normal", "swing");
                    } else {
                        document.cookie = "ZFDebugCollapsed=0;expires=;path=/";
                        jQuery("#Centurion_ZFDebug_toggler").html("&#171;");
                        return jQuery("#Centurion_ZFDebug_debug").animate({left:"5px"}, "normal", "swing");
                    }
                }
